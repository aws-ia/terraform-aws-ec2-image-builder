...
resource "aws_key_pair" "imagebuilder" {
  ...
  public_key = tls_private_key.imagebuilder[count.index].public_key_openssh
}

##Encrypted by default using KMS Key ID alias/aws/ssm
resource "aws_ssm_parameter" "imagebuilder_ssh_private_key_pem" {
  count = var.create_key_pair ? 1 : 0
  name  = "/${var.name}/imagebuilder_ssh_private_key_pem"
  type  = "SecureString"
  value = tls_private_key.imagebuilder[count.index].private_key_pem
}

##Encrypted by default using KMS Key ID alias/aws/ssm
resource "aws_ssm_parameter" "imagebuilder_ssh_public_key_pem" {
  count = var.create_key_pair ? 1 : 0
  name  = "/${var.name}/imagebuilder_ssh_public_key_pem"
  type  = "SecureString"
  value = tls_private_key.imagebuilder[count.index].public_key_pem
}

##Encrypted by default using KMS Key ID alias/aws/ssm
resource "aws_ssm_parameter" "imagebuilder_ssh_public_key_openssh" {
  count = var.create_key_pair ? 1 : 0
  name  = "/${var.name}/imagebuilder_ssh_public_key_openssh"
  type  = "SecureString"
  value = tls_private_key.imagebuilder[count.index].public_key_openssh
}
...
resource "aws_imagebuilder_infrastructure_configuration" "imagebuilder_infrastructure_configuration" {
  ...
  key_pair              = var.create_key_pair ? aws_key_pair.imagebuilder[count.index].key_name : var.instance_key_pair
